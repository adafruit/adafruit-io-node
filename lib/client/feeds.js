...
proto.connect = function() {
  ...
  this.client = mqtt.connect('mqtt://' + this.key + '@' + this.host + ':' + this.port);

  this.client.subscribe('api/feeds/' + this.id + '/streams/receive.json');
...
proto._write = function(data, encoding, cb) {
  ...
  this.client.publish('api/feeds/' + this.id + '/streams/send.json', data.toString());
  cb();

};

proto.streams = function(id, cb) {
...
proto.create_stream = function(data, cb) {
